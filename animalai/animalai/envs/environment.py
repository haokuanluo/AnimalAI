...
class UnityEnvironment(object):
    ...
    def executable_launcher(self, file_name, docker_training):
        ...
        if launch_string is None:
            ...
            #launch_string = "DISPLAY=:4 ." + launch_string
            print("This is the launch string",launch_string)
            if not docker_training:
                if self.play:
                    self.proc1 = subprocess.Popen(
                        ['DISPLAY=:4',launch_string, '--port', str(self.port)])
                    print("dfgfdgdfg",self.proc1)
                    ...
                    if self.resolution:
                        self.proc1 = subprocess.Popen(
                            [launch_string, '--port', str(self.port), '--resolution', str(self.resolution), '--nArenas',
                             str(self.n_arenas)], env={'DISPLAY':':4'})
                        print(self.proc1.pid)

                        # os.system("DISPLAY=:4 "+launch_string+' --port '+str(self.port)+
                        #                               ' --resolution '+ str(self.resolution)+ ' --nArenas '+str(self.n_arenas))
                        #self.proc1 = subprocess.Popen(launch_string+' --port '+str(self.port)+
                        #                             ' --resolution '+ str(self.resolution)+ ' --nArenas '+str(self.n_arenas),stdout=subprocess.PIPE, stderr=subprocess.PIPE ,env={'DISPLAY':'4'})
                        #self.proc1 = None
                        print("2222222", self.proc1)
                    else:
                        self.proc1 = subprocess.Popen(
                            [launch_string, '--port', str(self.port), '--nArenas', str(self.n_arenas)])
                        print("333333333", self.proc1)
